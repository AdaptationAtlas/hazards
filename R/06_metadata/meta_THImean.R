#Create Atlas metadata for THI: Cattle Thermal Humidity Index index (monthly mean). 
...
fnames <- gsub(pattern="/THI_mean", replacement="", x=fnames)
fnames <- gsub(pattern="_year_masked", replacement="", x=fnames)
fnames <- gsub(pattern="/THI/long_term_stats_mean", replacement="", x=fnames)
names(r_data) <- fnames

#write metadata
thi_meta <- atlas_metadata(data = r_data,
                           folder = meta_dir,
                           dataset.title_short = "Mean THI",
                           dataset.title_long = "Cattle thermal humidity index index, mean value of the month",
...
out_file <- file(paste0(meta_dir,"/THI_mean.txt"), open="w")
...
terra::writeRaster(r_data, paste0(buck_dir, "/THI_mean.tiff"), overwrite=TRUE)
