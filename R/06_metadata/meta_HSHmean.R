#Create Atlas metadata for HSH: Human Heat Stress index (monthly mean). This function 
...
fls <- c(sapply(paste0(his_dir, "/long_term_stats_mean/"), FUN=function(i) paste0(i, fls)),
         sapply(paste0(ssp_dir, "/long_term_stats_mean/"), FUN=function(i) paste0(i, fls)),
         sapply(paste0(ssp_dir, "_mean/"), FUN=function(i) paste0(i, fls)))
...
fnames <- gsub(pattern="/HSH_mean", replacement="", x=fnames)
fnames <- gsub(pattern="_year_masked", replacement="", x=fnames)
fnames <- gsub(pattern="/HSH/long_term_stats_mean", replacement="", x=fnames)
names(r_data) <- fnames

#write metadata
hsh_meta <- atlas_metadata(data = r_data,
                           folder = meta_dir,
                           dataset.title_short = "Mean HSH",
                           dataset.title_long = "Human heat stress index, mean value of the month",
                           dataset.desc = "The heat index (HI) was calculated from daily data and empirical equations based on Steadman (1979a; 1979b) relating the HI to dry-bulb temperature (°C) and relative humidity (%). Using the daily values the mean of each month is first calculated, and following that we calculate the climatological mean of each month, and then the maximum, mean and median of the year.",
                           ...
                           variable.subname = "Mean of month",
...
out_file <- file(paste0(meta_dir,"/HSH_mean.txt"), open="w")
...
terra::writeRaster(r_data, paste0(buck_dir, "/HSH_mean.tiff"), overwrite=TRUE)
