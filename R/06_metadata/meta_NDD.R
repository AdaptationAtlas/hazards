...
#directories
his_dir <- paste0(wd, "/indices/historical/NDD")
ssp_dir <- paste0(wd, "/indices/", full_tb$fullname, "/NDD")
...
fnames <- gsub(pattern="/NDD/long_term_stats", replacement="", x=fnames)
fnames <- gsub(pattern="/NDD", replacement="", x=fnames)
names(r_data) <- fnames

#write metadata
ndd_meta <- atlas_metadata(data = r_data,
                           folder = meta_dir,
                           dataset.title_short = "NDD",
                           dataset.title_long = "Total number of dry days (NDD) per month",
                           dataset.desc = "Number of dry days (P < 1 mm) per month and per year for historical and future periods, calculated using CHIRPS daily precipitation data.",
                           ...
                           variable.name = "number of dry days",
                           ...
                           method.description = "Count the number of dry days (i.e., accumulated daily precipitation < 1 mm) per month.",
...
out_file <- file(paste0(meta_dir,"/NDD.txt"), open="w")
for (i in 1:ncol(ndd_meta)) {
  writeLines(text=names(ndd_meta)[i], con=out_file)
  writeLines(text=paste0(ndd_meta[1,i], "\n"), con=out_file)
}
...
terra::writeRaster(r_data, paste0(buck_dir, "/NDD.tiff"), overwrite=TRUE)
