...
#directories
his_dir <- paste0(wd, "/indices/historical/NTx40")
ssp_dir <- paste0(wd, "/indices/", full_tb$fullname, "/NTx40")
...
fnames <- gsub(pattern="/NTx40/long_term_stats", replacement="", x=fnames)
fnames <- gsub(pattern="/NTx40", replacement="", x=fnames)
names(r_data) <- fnames

#write metadata
ntx_meta <- atlas_metadata(data = r_data,
                           folder = meta_dir,
                           dataset.title_short = "NTx40",
                           dataset.title_long = "Total number of heat stress days per month (generic crop)",
                           dataset.desc = "The number of days with daily maximum temperatures above a given threshold (40 Celsius degrees). No crop calendar is used (i.e., we calculate it for the whole year).",
                           ...
                           variable.name = "number of heat stress days (generic crop)",
                           ...
                           method.description = "The number of days with daily maximum temperatures above 40 Celsius degrees per month. To compute this, no crop calendar is used.",
...
out_file <- file(paste0(meta_dir,"/NTx40.txt"), open="w")
...
terra::writeRaster(r_data, paste0(buck_dir, "/NTx40.tiff"), overwrite=TRUE)
