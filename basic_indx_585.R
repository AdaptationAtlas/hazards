...
calc_AgrClm <- function(season = season, shp_fl = shp_fl, iso = iso){
  ...
  outfile <- paste0(root, '/7.Results/Africa_final/',PRD,'/',ssp,'/',iso); if(!dir.exists(outfile)){dir.create(outfile, F, T)}
  ...
  ## Daily files
  # Precipitation
  pr_pth <- paste0(root,'/7.Results/future_africa/ensamble/', PRD, '/', iso, '/', ssp)
  pr <- terra::rast(paste0(pr_pth, '/','pr.tif'))
  # pr_dts <- gsub(pattern='chirps.v2.0.', replacement= '', x = basename(names(pr)))
  # pr_dts <- gsub(pattern='.', replacement= '-', x = pr_dts, fixed = T)
  # pr_dts <- as.Date(pr_dts)
  pr_fls <- pr
  names(pr_fls[[1:2192]]) <- dts
  pr_dts <- as.Date(names(pr_fls))
  # pr_fls <- pr[[which(pr_dts %in% dts)]]
  
  # Tmax
  tmx_pth <- paste0(root,'/7.Results/future_africa/ensamble/', PRD, '/', iso, '/', ssp)
  tmx <- terra::rast(paste0(tmx_pth, '/','tasmax.tif'))
  # tmx_dts <- gsub(pattern='chirts-v2.0.', replacement= '', x = basename(names(tmx)))
  # tmx_dts <- gsub(pattern='.', replacement= '-', x = tmx_dts, fixed = T)
  # tmx_dts <- as.Date(tmx_dts)
  tmx_fls <- tmx
  names(tmx_fls[[1:2192]]) <- dts
  tmx_dts <- as.Date(names(tmx_fls))
  # tmx_fls <- tmx[[which(tmx_dts %in% dts)]]

  # Tmin
  tmn_pth <- paste0(root,'/7.Results/future_africa/ensamble/', PRD, '/', iso, '/', ssp)
  tmn <- terra::rast(paste0(tmn_pth, '/','tasmin.tif'))
  # tmn_dts <- gsub(pattern='chirts-v2.0.', replacement= '', x = basename(names(tmn)))
  # tmn_dts <- gsub(pattern='.', replacement= '-', x = tmn_dts, fixed = T)
  # tmn_dts <- as.Date(tmn_dts)
  tmn_fls <- tmn
  names(tmn_fls[[1:2192]]) <- dts
  tmn_dts <- as.Date(names(tmn_fls))
  # tmn_fls <- tmn[[which(tmn_dts %in% dts)]]
  ...
  tmx_fls <- tmx_fls[[which(lubridate::year(tmx_dts) %in% yrs)]]
  tmn_fls <- tmn_fls[[which(lubridate::year(tmn_dts) %in% yrs)]]
  ...
  IND <- 1:length(yrs_dts) %>%
    purrr::map(.f = function(i){
      ...
      tmx <- (tmx_fls[[which(tmx_dts %in% yrs_dts[[i]])]])
      tmx <- tmx %>% terra::crop(terra::ext(tmp)) %>% terra::mask(tmp)
      
      prc <- (pr_fls[[which(pr_dts %in% yrs_dts[[i]])]])
...
GCM <- ('ACCESS-ESM1-5') # ACCESS-ESM1-5 EC-Earth3-Veg INM-CM5-0 MPI-ESM1-2-HR MRI-ESM2-0
PRD <- '2030s' # 2050s
